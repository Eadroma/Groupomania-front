...
import { addItemLocalStorage, readLocalStorage } from "../../components/UserProvider";
...
export default function Signup() {
    const handleForm = async (event) => {
        ...
        const apiUrl = 'https://groupomania-myback.herokuapp.com/api/auth/register'
        ...
        if (!(result.id && result.token)) {
            document.getElementById('error-msg').textContent('Nous rencontrons un problème..')
        } else {
            const localUser = {
                id: result.id,
                token: result.token,
...
<div id="container" className={styles.container}>
    ...
    <h1 class="title" style={{color: '#00d1b2', marginBottom: '5rem', width: '50%', margin: 'auto', textAlign: 'center'}}>Signup</h1>
        <form class="box" id="form" onSubmit={handleForm}>
            <div class="field">
                <label class="label" htmlFor="email">Email</label>
                <div class="control">
                    <input class="input" id="email" name="email" type="email" placeholder="e.g. alex@example.com" required />
                </div>
            </div>
